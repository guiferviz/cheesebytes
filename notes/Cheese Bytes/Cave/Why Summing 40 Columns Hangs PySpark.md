...
Because **the code is actually stuck during the `select()` step**, _before_ any action happens.
...
The problem lies in the **analysis phase**, when Spark tries to _prepare_ the expression. If you inspect the `expr` variable right after defining it:
