...
const DrawingOverlay: React.FC<DrawingOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        e.key.toLowerCase() === activationKey.toLowerCase() &&
        !e.ctrlKey &&
        !e.metaKey
      ) {
        e.preventDefault();
    ...
    const handleToggleEvent = () => {
      setIsActive((prev) => {
        if (prev) setIsToolbarOpen(false);
        return !prev;
      });
    };
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`flex items-center gap-3 p-3 backdrop-blur-sm rounded-2xl shadow-2xl border transition-colors duration-300 ${
            isDarkMode
        ...
        >
          ...
          <div
            className={`w-px h-8 ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`}
          />

          {/* Tools section */}
          <div
            className={`flex items-center gap-1.5 pr-3 border-r ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
          >
            {(["pen", "rectangle", "circle", "eraser"] as Tool[]).map(
              (tool) => (
                <button
                  key={tool}
          ...
          {/* Colors section */}
          <div
            className={`flex items-center gap-2 px-3 border-r ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
          >
            <span
      ...
      >
        🎨 Drawing mode ·{" "}
        <kbd
          className={`px-1.5 py-0.5 rounded text-xs ${isDarkMode ? "bg-white/20" : "bg-gray-200"}`}
        >
          P
        </kbd>{" "}
        to exit
