...
const ELEMENT_NAMES: Record<string, string> = Object.fromEntries(
  elementsData.map((e) => [e.symbol, e.nameEn]),
);
...
export const ElementalTree: React.FC<ElementalTreeProps> = ({
  ...
  const buildTreeData = useCallback(
    (word: string, depth = 0, parentId = "root"): TreeNode | null => {
      // If we've exceeded the depth we want to visualize, we stop building the tree content,
      // but typically we control depth via the recursion logic below.
      // However, if we just want to stop *calculating*, we can check depth here.
      // Note: The original code allowed going deeper if needed or checked depth in loop.
      // Let's stick to the logic: we build nodes. If depth == maxDepth, we return the node but don't recurse.
      ...
      if (ELEMENT_SYMBOLS.has(symbol1)) {
        ...
        node.children.push(
          child
            ? { ...child, symbol: symbol1 }
            : ({ ...DEAD_END_NODE, symbol: symbol1 } as TreeNode),
        );
      } else {
      ...
      if (word.length >= 2) {
        ...
        if (ELEMENT_SYMBOLS.has(symbol2)) {
          ...
          node.children.push(
            child
              ? { ...child, symbol: symbol2 }
              : ({ ...DEAD_END_NODE, symbol: symbol2 } as TreeNode),
          );
        } else {
  ...
  useEffect(() => {
    ...
    if (maxDepth < 99) {
      const treeData = buildTreeData(
        (showInput ? name : initialText).toLowerCase(),
      );
  ...
  const stableHighlightBranchIds = useMemo(
    () => highlightBranchIds,
    [JSON.stringify(highlightBranchIds)],
  );
  ...
  useEffect(() => {
    ...
    const nodeEnter = nodeSel
      ...
      .attr(
        "transform",
        (d: any) =>
          `translate(${prevPos.current[d.id]?.y ?? d.y},${prevPos.current[d.id]?.x ?? d.x})`,
      )
    ...
    nodeEnter
      ...
      .attr("class", (d: any) => {
        switch (d.data.status) {
          case "success":
            return "node-rect-success";
          case "dead_end":
            return "node-rect-dead_end";
          default:
            return "node-rect-intermediate";
        }
      })
      .attr(
        "width",
        (d: any) => (d.data.text ? d.data.text.length : 1) * 9 + 20,
      )
      .attr("height", 30)
      .attr(
        "x",
        (d: any) => -((d.data.text ? d.data.text.length : 1) * 4.5 + 10),
      )
      .attr("y", -15)
      .attr("rx", 8);
    nodeEnter
    ...
    if (
      highlightBranchIds &&
      highlightBranchIds.length > 0 &&
      currentHighlightedIds.size > 0
    ) {
    ...
    const linkEnter = linkSel
      ...
      .style("stroke-dasharray", (d: any) =>
        d.target.data.status === "dead_end" ? "3,4" : "none",
      );
    // Sin animación: solo setear el path directamente
    linkSel.merge(linkEnter as any).attr(
      "d",
      d3
        .linkHorizontal()
        .x((d: any) => d.y)
    ...
    d3.select(linksRef.current)
      ...
      .on("mouseenter", function (event: any, d: any) {
        ...
        d3.select(gRef.current)
          ...
          .attr(
            "transform",
            `translate(${(d.source.y + d.target.y) / 2},${(d.source.x + d.target.x) / 2 - 30})`,
          )
          .append("rect")
          .attr("x", -80)
          .attr("y", -28)
