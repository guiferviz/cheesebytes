...
const elementMap: Record<string, ElementData> = Object.fromEntries(
  elementsData.map((e) => [e.symbol.toLowerCase(), e]),
);
...
> = ({ initialText = "Snack", showInput = true, maxCombinations = 30 }) => {
  ...
  const maxElements = combosToShow.reduce(
      ...
      {showInput && (
        <div className="mb-8 text-center" style={{ flexShrink: 0 }}>
          <input
            ...
            placeholder="Enter an elemental name..."
          />
        </div>
      )}
      <div
      ...
      >
        ...
        {combinations.length > 30 && (
          <div
            ...
            style={{ flexShrink: 0, padding: "8px 0" }}
          >
            Showing only the first 30 combinations of {combinations.length}{" "}
            possible.
          </div>
        )}
